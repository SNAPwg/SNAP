                   ...
                   Theme = theme_update())
{
  ...
  {
    LengthData <- Data[grepl('_LengthData',Files)]
  }
  if ( any(grepl('_CatchData',Files)) )
  {
    CatchData <-  Data[grepl('_CatchData',Files)]
  }
  if ( any(grepl('_EffortData',Files)) )
  {
    EffortData <- Data[grepl('_EffortData',Files)]
  }
  if ( any(grepl('_CPUEData',Files)) )
  {
    CPUEData <- Data[grepl('_CPUEData',Files)]
  }
  if ( any(grepl('_DensityData',Files)) )
  {
    DensityData <- Data[grepl('_DensityData',Files)]
  }
  if ( any(grepl('_MapData',Files)) )
  {
    Locations <- Data[grepl('_MapData',Files)]
  }
  ...
  Fishery$CommName <- 'Lobster'

  Fishery$Taxa <- 'Crustacean'

    LifeHistory <- with(Fishery,
                data.frame(Species = Species, CommName = CommName, Taxa = Taxa,
                           HasLifeHistory = 1, Linf = Linf, vbk = K, t0 = t0,
                           MaxAge = kmax, AgeMat50 = kmat, LengthMatRatio = NA, Mat50 = NA,
                           WeightA = wtA, WeightB = wtB ))
